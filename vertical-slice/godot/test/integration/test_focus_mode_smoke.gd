...
func test_signal_connections():
	...
	assert_true(
		main.has_signal("world_listeners_updated"), "world_listeners_updated signal missing"
	)
	...
	if disruptor:
		...
		assert_true(
			disruptor.is_connected("chaos_pulse", target),
			"chaos_pulse should be connected to _on_chaos"
		)
	...
	if observer_light:
		if observer_light.has_signal("tuned_in"):
			...
			assert_true(
				observer_light.is_connected("tuned_in", target_light),
				"tuned_in should be connected to _update_world_listeners"
			)
...
func test_clue_resolve_requires_calm_in_tune_in() -> void:
	...
	assert_true(
		main.clue_resolve_progress > 0.5,
		"Clue resolve progress should advance under high calm in TuneIn"
	)
	...
	main.call("_update_clue_resolution", 0.5)
	assert_almost_eq(
		main.clue_resolve_progress, 0.0, 0.01, "Clue resolve progress should decay outside TuneIn"
	)
...
func test_evidence_board_progress_and_contradiction() -> void:
	...
	board.contradiction_detected.connect(
		func(a: String, b: String) -> void: received_contradiction = true
	)
	resolver_a.apply_presence(1.0)
	resolver_contra.apply_presence(1.0)
	assert_true(
		received_contradiction,
		"Contradiction should be detected when both conflicting clues resolve"
	)

	var last_progress := 0.0
	board.deduction_progress.connect(
		func(progress: float, _text: String) -> void: last_progress = progress
	)
	resolver_a.apply_presence(1.0)
	resolver_b.apply_presence(1.0)
...
func test_evidence_board_wired_in_focus_mode() -> void:
	...
	if smudge and neon:
		...
		var progress_values = []
		main.deduction_updated.connect(
			func(p: float, _t: String) -> void: progress_values.append(p)
		)
		...
		assert_true(
			progress_values.back() > 0.9, "Progress should be high when both clues resolved"
		)
...
func test_investigation_beat_resolves_after_combined_clues() -> void:
	...
	var resolved_texts = []
	beat.beat_resolved.connect(func(text: String) -> void: resolved_texts.append(text))
	smudge.apply_presence(1.0)
	neon.apply_presence(1.0)
	smudge._clarity = 1.0
	neon._clarity = 1.0
	...
	neon.clarity_changed.emit("neon", 1.0)
	assert_true(
		resolved_texts.size() > 0, "beat_resolved should emit when combined clues reach threshold"
	)
	if resolved_texts.size() > 0:
		assert_true(
			resolved_texts.back() != "", "beat_resolved should include non-empty insight text"
		)
...
func test_sensory_crime_loop_emits_phase_without_focus_main() -> void:
	...
	var phases = []
	loop.phase_changed.connect(func(from: int, to: int) -> void: phases.append([from, to]))
	...
	assert_eq(
		loop.current_phase, SensoryCrimeLoop.Phase.TUNE_IN, "Loop should own its current phase"
	)
...
func test_sensory_crime_loop_resets_on_any_reset_signal() -> void:
	...
	assert_eq(
		...
		"Loop should reset when reset signal fires"
	)
...
func test_sensory_crime_loop_resolves_when_beat_resolves() -> void:
	...
	var resolved_texts = []
	loop.loop_resolved.connect(func(text: String) -> void: resolved_texts.append(text))

	var beat = main.investigation_beat
	var combo_texts = []
	beat.beat_resolved.connect(func(text: String) -> void: combo_texts.append(text))
...
func test_audio_bus_manager_looks_up_effects_by_name() -> void:
	...
	assert_true(
		manager._effects.has("highpass"), "AudioBusManager should register highpass by name"
	)
	assert_true(
		manager._effects.has("bandpass"), "AudioBusManager should register bandpass by name"
	)
...
func test_audio_bus_manager_glide_noops_when_effect_missing() -> void:
	...
	assert_almost_eq(
		manager._current_band_cutoff,
		1000.0,
		0.01,
		"Glide should no-op when effect lookup returns null"
	)
...
func test_vignette_calculation_constants_present() -> void:
	...
	assert_true(
		canvas.has_method("_get_vignette_strength"), "Vignette strength helper should exist"
	)
...
func test_disruptor_auto_fire_default_false() -> void:
	...
	assert_false(
		disruptor_node.is_active(),
		"Disruptor should not start automatically when auto_fire is false"
	)
...
func test_disruptor_missing_variant_does_not_crash() -> void:
	...
	assert_false(
		disruptor_node.has_signal("chaos_pulse"),
		"No signal should exist; trigger_pulse should no-op when variant is absent"
	)
...
func test_disruptor_variant_emits_chaos_pulse() -> void:
	...
	var strengths = []
	disruptor_node.chaos_pulse.connect(func(value: float) -> void: strengths.append(value))
	disruptor_node.trigger_pulse()
...
func test_audio_bus_manager_apply_chaos_band_shifts_targets() -> void:
	...
	assert_true(
		manager._target_band_cutoff < 1200.0 - 0.01, "Low band should lower bandpass target cutoff"
	)

	manager.apply_chaos_band("high", 1.0)
	assert_true(
		manager._target_band_cutoff > 1200.0 + 0.01, "High band should raise bandpass target cutoff"
	)
...
func test_focus_mode_main_rich_chaos_applies_audio_band() -> void:
	...
	manager.apply_chaos_band = func(band: String, strength: float) -> void: band_requested = band

	main._on_chaos_rich(0.4, 0.5, "mid")
	assert_eq(
		band_requested,
		"mid",
		"FocusModeMain should delegate band to AudioBusManager.apply_chaos_band"
	)
...
func test_clue_graph_register_and_clarity() -> void:
	...
	assert_almost_eq(
		graph.get_clarity("smudge_pattern"), 0.2, 0.01, "ClueGraph should persist clarity"
	)
...
func test_clue_graph_resolve_locks_clue() -> void:
	...
	graph.clue_unlocked.connect(
		func(from_clue: String, ids: Array[String]) -> void: unlocked_ids = ids
	)
...
func test_evidence_board_bridges_to_clue_graph() -> void:
	...
	var graph = ClueGraph.new()
	add_child_autofree(graph)
	board.graph_progression_requested.connect(func(id: String) -> void: graph.resolve_clue(id))
	var data = ClueData.new()
...
func test_disruptor_profile_activated_signal() -> void:
	...
	d.profile = profile
	var emitted = []
	d.profile_activated.connect(func(p: DisruptorProfile) -> void: emitted.append(p))
	d._ready()
	assert_eq(
		emitted.size(),
		1,
		"Disruptor should emit profile_activated on ready when profile is assigned"
	)
...
func test_focus_mode_main_assigns_disruptor_profile() -> void:
	...
	assert_true(
		d.profile != null, "FocusModeMain should assign a DisruptorProfile to the disruptor"
	)
...
func test_sensory_canvas_uses_colorblind_trail_tint() -> void:
	...
	var received = []
	canvas.trail_proximity.connect(func(value: float) -> void: received.append(value))
	canvas.call("set_trail_target", Vector2(11, 11))
	assert_true(
		received.size() > 0, "trail_proximity should emit when trail target is near trail points"
	)
	assert_true(
		received.back() > 0.0, "trail_proximity should report nonzero proximity near the trail"
	)
...
func test_observer_light_chaos_flicker_sets_flicker_state() -> void:
	...
	assert_true(
		light_node.flicker_strength,
		"Chaos spike should set flicker_strength before next calm apply"
	)
...
func test_npc_regular_startle_resets_to_origin() -> void:
	...
	assert_true(
		(npc.position - start_pos).length() < 1e-3, "NPC should return near origin after recover"
	)
...
func test_focus_mode_main_world_listeners_updated_emits_presence() -> void:
	...
	var main = scene
	var received = []
	main.world_listeners_updated.connect(func(value: float) -> void: received.append(value))
	main.presence = 0.42
	main.call("_update_world_listeners", 0.05)
	assert_true(
		received.size() > 0,
		"world_listeners_updated should emit each frame from _update_world_listeners"
	)
	assert_almost_eq(
		received.back(), 0.42, 0.01, "world_listeners_updated should emit the current presence"
	)
...
func test_focus_mode_resolve_routes_through_evidence_board() -> void:
	...
	var main = scene
	var signals = []
	main.evidence_board.graph_progression_requested.connect(
		func(id: String) -> void: signals.append(id)
	)
	main.investigation_phase = main.InvestigationPhase.TuneIn
	...
	assert_eq(
		signals.size(),
		1,
		"EvidenceBoard.graph_progression_requested should fire once per active resolve"
	)
...
func test_focus_mode_main_applies_clue_profile_on_overload() -> void:
	...
	var main = scene
	var active_signal = ""
	main.evidence_board.deduction_progress.connect(
		func(progress: float, _text: String) -> void: active_signal = "progress"
	)
	main.disruptor.profile_activated.connect(func(profile) -> void: active_signal = profile.id)
	main.active_clue_id = "smudge"
	main.investigation_phase = main.InvestigationPhase.TuneIn
	main.call("_apply_disruptor_profile_for_active_clue")
	assert_eq(
		main.disruptor.profile.id if main.disruptor.profile else "",
		"acoustic_bleed",
		"Active clue should map to its DisruptorProfile"
	)
	...
	assert_eq(
		main.disruptor.profile.id if main.disruptor.profile else "",
		"overload_artist",
		"Neon clue should use overload_artist profile"
	)
